...
class AlignedDataset_Rand_Seg(BaseDataset):
    def initialize(self, opt):
        ...
        self.dir_A =   os.path.join(opt.dataroot, opt.phase + 'D_temp1')
        self.dir_B =   os.path.join(opt.dataroot, opt.phase + 'D_temp2')
        self.dir_A_Seg=os.path.join(opt.dataroot, opt.phase + 'D_temp1_seg')
        self.dir_B_Seg=os.path.join(opt.dataroot, opt.phase + 'D_temp2_seg')
        self.dir_A_Map=os.path.join(opt.dataroot, opt.phase + 'D_temp1_segmap')
        self.dir_B_Map=os.path.join(opt.dataroot, opt.phase + 'D_temp2_segmap')
        ...
        self.A_paths_seg=make_dataset(self.dir_A_Seg)
        self.B_paths_seg=make_dataset(self.dir_B_Seg)
        self.A_paths_map=make_dataset(self.dir_A_Map)
        self.B_paths_map=make_dataset(self.dir_B_Map)

        self.A_paths = sorted(self.A_paths)
        self.B_paths = sorted(self.B_paths)
        self.A_paths_seg=sorted(self.A_paths_seg)
        self.B_paths_seg=sorted(self.B_paths_seg)
    ...
    def __getitem__(self, index):
        ...
        A_path_seg=self.A_paths_seg[index % self.A_size]
        B_path_seg=self.B_paths_seg[index % self.A_size]
        A_path_map=self.A_paths_map[index % self.A_size]
        B_path_map=self.B_paths_map[index % self.A_size]
        ...
        if output_nc == 1:  # RGB to gray
            ...
            B = tmp.unsqueeze(0)
        return {'A': A, 'B': B, 'C': C, 'A_seg': A_path_seg, 'B_seg': B_path_seg, 'A_map': A_map, 'B_map': B_map,
    ...
    def name(self):
        return 'AlignedDataset_Rand_Seg'

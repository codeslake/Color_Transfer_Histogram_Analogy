...
class AlignedDataset_Rand_Seg_onlymap(BaseDataset):
    def initialize(self, opt):
        ...
        # self.dir_A_Map=os.path.join(opt.dataroot, opt.phase + '{}_1_segmap'.format(opt.test_dir))
        # self.dir_B_Map=os.path.join(opt.dataroot, opt.phase + '{}_2_segmap'.format(opt.test_dir))
        ...
        #self.dir_A =   os.path.join(opt.dataroot, opt.phase + 'Z_temp1')
        #self.dir_B =   os.path.join(opt.dataroot, opt.phase + 'Z_temp2')
        #self.dir_A_Map=os.path.join(opt.dataroot, opt.phase + 'Z_temp1_segmap3')
        #self.dir_B_Map=os.path.join(opt.dataroot, opt.phase + 'Z_temp2_segmap3')

        #self.dir_A =   os.path.join(opt.dataroot, opt.phase + 'H_temp1')
        #self.dir_B =   os.path.join(opt.dataroot, opt.phase + 'B_exC_resized')
        #self.dir_A_Map=os.path.join(opt.dataroot, opt.phase + 'H_temp1_segmap')
        #self.dir_B_Map=os.path.join(opt.dataroot, opt.phase + 'A_segmap')

        #self.dir_A =   os.path.join(opt.dataroot, opt.phase + 'B_exC_resized')
        #self.dir_B =   os.path.join(opt.dataroot, opt.phase + 'I_temp2')
        #self.dir_A_Map=os.path.join(opt.dataroot, opt.phase + 'A_segmap')
        #self.dir_B_Map=os.path.join(opt.dataroot, opt.phase + 'I_temp2_segmap')
    ...
    def __getitem__(self, index):
        ...
        if output_nc == 1:  # RGB to gray
            ...
            B = tmp.unsqueeze(0)
        return {'A': A, 'B': B, 'C': C, 'A_map': A_map, 'B_map': B_map,
